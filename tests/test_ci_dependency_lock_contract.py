...
def test_ci_workflow_has_fast_full_dependency_and_redis_acceptance_jobs():
    ...
    redis_job = jobs["redis-runtime-acceptance"]
    assert redis_job["services"]["redis"]["image"] == "redis:7.4.0-alpine"


def test_redis_acceptance_uses_real_service_hash_locked_client_and_load_evidence():
    text = (ROOT / ".github/workflows/ci.yml").read_text(encoding="utf-8")
    assert "redis:7.4.0-alpine" in text
    assert 'redis-cli ping' in text
    assert "./scripts/lock_dependencies.sh agent-redis" in text
    assert ".ci-locks/agent-redis-py311-linux.lock.txt" in text
    assert "tests/runtime/test_agent_redis_runtime_acceptance.py" in text
    assert "scripts/run_agent_redis_load.py" in text
    assert "--profile ci-smoke" in text
    assert "agent-redis-load-smoke.json" in text
    assert "actions/upload-artifact@v7" in text
    assert "redis-runtime-acceptance.xml" in text


def test_manual_slo_calibration_lab_is_read_only_and_separate_from_push_ci():
    workflow = _load_yaml(
        ROOT / ".github/workflows/agent-slo-calibration.yml"
    )
...
def test_lock_policy_covers_every_default_runtime_environment():
    policy = yaml.safe_load(
        (ROOT / "requirements/locks/LOCK_POLICY.yml").read_text(encoding="utf-8")
    )
...
def test_lock_script_preserves_runtime_boundaries_and_optional_redis_lock():
    ...
    assert 'agent-redis) echo "requirements-agent-redis.txt"' in text
...
def test_dependency_lock_workflow_generates_and_validates_all_default_locks():
    workflow = _load_yaml(
        ROOT / ".github/workflows/dependency-locks.yml"
    )
    assert workflow["permissions"]["contents"] == "read"
